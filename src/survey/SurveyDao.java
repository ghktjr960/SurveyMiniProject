...
public class SurveyDao {
	...
	public Map<String, Integer> selectParticipantGender(String column, String table) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Map<String, Integer> result = new HashMap<String, Integer>();
		HashSet<String> genderValueSet = new HashSet<>();
		
		try {
			conn = util.getConnection();
			String joinTable = "select A.\"GENDER\", B.\"BLUEEXPERIENCE\" from \"SURVEYINFO\" A join \"SURVEYVALUE\" B on A.\"NUM\" = B.\"NUM\"";
			String query = "select count(\"GENDER\") as \"GENDERCOUNT\" from (" + joinTable + ") where \"GENDER\" = ?";
			genderValueSet = surveyValueSet(column, table);
			List<String> genderValueList = new ArrayList<>(genderValueSet);
			
			for (int i = 0; i < genderValueList.size(); i++) {
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, genderValueList.get(i));
				rs = pstmt.executeQuery();
				if(rs.next()) {
					result.put(genderValueList.get(i), rs.getInt("GENDERCOUNT"));
					pstmt.close();
					rs.close();
				}
			}
	...
	public Map<String, Integer> selectParticipantAge(String column, String table) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Map<String, Integer> result = new HashMap<>();
		HashSet<String> ageValueSet = new HashSet<>();
		
		try {
			conn = util.getConnection();
			String joinTable = "select A.\"AGE\", B.\"BLUEEXPERIENCE\" from \"SURVEYINFO\" A join \"SURVEYVALUE\" B on A.\"NUM\" = B.\"NUM\"";
			String query = "select count(\"AGE\") as \"AGECOUNT\" from (" + joinTable + ") where \"AGE\" = ?";
			ageValueSet = surveyValueSet(column, table);
			List<String> genderValueList = new ArrayList<String>(ageValueSet);
			for (int i = 0; i < genderValueList.size(); i++) {
				pstmt = conn.prepareStatement(query);
				pstmt.setInt(1, Integer.parseInt(genderValueList.get(i)));
				rs = pstmt.executeQuery();
				if(rs.next()) {
					result.put(genderValueList.get(i), rs.getInt("AGECOUNT"));
					pstmt.close();
					rs.close();
				} else {
				}
